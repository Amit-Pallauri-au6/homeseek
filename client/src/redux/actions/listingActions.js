...
export const listingHouse = (details) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(`/owner/listing/create`, details, {headers: headers})
        //console.log(data)
        dispatch({
            type: LISTING_HOUSE,
            payload: data
...
export const createOTP = (data2) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(`/owner/listing`, data2, {headers: headers})
        //console.log(data)
...
export const verifyOTP = (data1) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(`/owner/listing/verify`, data1 , {headers: headers})
        dispatch({
            type: VERIFY_OTP,
            payload: data
        })
...
export const allHomes = () => async (dispatch, getState) => {
    try {
            ...
            const { data } = await axios.get(`/listings`, {headers: headers})
            dispatch({
                type: GETALLPOSTS,
                payload: data
            })
...
export const particularHome = (homeId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/owner/home/${homeId}`, {headers: headers})
        //console.log(homeId)
        dispatch({
...
export const getAllSortedPosts = () => async dispatch => {
   try {
       ...
       const { data } = await axios.get(`/getAllSortedPosts`, {headers})
       dispatch({
           type :  SORTED_VALUES,
           payload : data
       })
...
export const getFilteredData = details => async dispatch => {
    try {
        ...
        const { data } = await axios.get(`/filter`, { headers, params: { 
            location,
            maxRent, 
            minRent,
            type 
...
export const getAllVerifiedPosts = () => async dispatch => {
    try {
        ...
        const { data } = await axios.get(`/getAllPostedListings`, {headers})
        dispatch({
...
export const createOwnerRequests = details => async dispatch => {
    try {
        ...
        const { data } = await axios.post(`/createOwnerRequests`, details, {headers})
