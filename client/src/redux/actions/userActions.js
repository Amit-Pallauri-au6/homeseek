...
export const registerUser = user => async dispatch =>  {
    try {
        ...
        const { data } = await axios.post(`/signup`, user, {headers: headers})
        dispatch({
            type: REGISTER_USER,
            payload: data
        })   
...
export const loginUser = user => async (dispatch) =>  {
    try {
        ...
        const { data } = await axios.post(`/signIn`, user, {headers: headers});
...
export const logoutUser = ()  => async dispatch => {
    try {
        ...
        const { data } = await axios.delete(`/signOut`, { headers : headers })
        console.log(data)
        dispatch({
            type : LOGOUT_USER
        })
...
export const loginViaThirdParty = details => async dispatch => {
    try {
        ...
        const { data } = await axios.post(`/thirdPartysignIn`, {details} , {headers})
        dispatch({
...
export const forgotPassword = email => async dispatch => {
    try {
        ...
        const {data} = await axios.post(`/forgotPassword`,{email}, { headers })
        dispatch({
            type : FORGOT_PASS,
            payload : data
        })
...
export const addProfilepic = state => async dispatch => {
    try {
        ...
        const { data } = await axios.post(`/uploadPic`, state, { headers })
        dispatch({
...
export const updateProfile = details => async dispatch => {
    try {
        ...
        const { data } = await axios.post(`/addDetails`, { DOB, gender, Address, maritalStatus }, { headers })
        dispatch({
            type : UPDATE_DETAILS,
            payload : data
        })
...
export const addServiceRequest  =  req => async dispatch => {
    try {
        ...
        const { data } = await axios.post(`/user/request`, { request, description }, {headers})
...
export const addNormalRequest  =  req => async dispatch => {
    try {
        ...
        const { data } = await axios.post(`/user/book/request`, req , {headers})
        dispatch({
            type : NORMAL_REQUEST,
            payload : data  
        })
    ...
    export const updateBankDetails = details => async dispatch => {
        ...
        const { data } = await axios.post(`/updateBankDetails`, { details }, { headers })
        dispatch({
    ...
    export const getMyHome = () => async dispatch => {
        try {
            ...
            const { data } = await axios.get(`/getMyHome`, {headers})
            dispatch({
                type : GET_MY_HOME,
                payload : data
            })
